package com.example.cryptoproject.Cipher;

public class Kyznechik {
    ...
    public void keys(byte[] key)
    {
        byte[] key_1 = new byte[16];
        byte[] key_2 = new byte[16];
        System.arraycopy(key, 0, key_1, 0, BLOCK_SIZE);
        System.arraycopy(key, 16, key_2, 0, BLOCK_SIZE);
        GOST_Kuz_Expand_Key(key_1, key_2);
    }
    public void setiv(byte[] arr)
    {
        iv = arr;
    }
    ...
    static private byte[] mod2(byte[] a, byte[] b)
    {
        int i;
    ...
    public byte[] updateenc(byte[] arr)
    {
        byte[] mas = new byte[arr.length];
        int n = arr.length / BLOCK_SIZE;
        byte[] massiv = new byte [16];
        System.arraycopy (arr, 0, massiv, 0, BLOCK_SIZE);
        massiv = mod2(massiv, iv);
        massiv = GOST_Kuz_Encript(massiv);
        System.arraycopy (massiv, 0, mas, 0, BLOCK_SIZE);
        byte[] massthis = new byte [16];
        byte[] masspast = new byte [16];
        for (int i = 1; i < n; i++)
        {
            System.arraycopy (arr, i * BLOCK_SIZE, massthis, 0, BLOCK_SIZE);
            System.arraycopy (mas, (i - 1) * BLOCK_SIZE, masspast, 0, BLOCK_SIZE);
            massthis = mod2(massthis,masspast);
            massthis = GOST_Kuz_Encript(massthis);
            System.arraycopy (massthis, 0, mas, i * BLOCK_SIZE, BLOCK_SIZE);
            massthis = new byte [16];
            masspast = new byte [16];
        }
        return mas;
    }

    public byte[] updatedec(byte[] arr)
    {
        byte[] mas = new byte[arr.length];
        int n = arr.length / BLOCK_SIZE;
        byte[] massthis = new byte[16];
        ...
        {
            System.arraycopy (arr, i * BLOCK_SIZE, massthis, 0, BLOCK_SIZE);
            System.arraycopy (arr, (i - 1) * BLOCK_SIZE, masspast, 0, BLOCK_SIZE);
            massthis = GOST_Kuz_Decript(massthis);
            massthis = mod2(massthis,masspast);
            System.arraycopy (massthis, 0, mas, i * BLOCK_SIZE, BLOCK_SIZE);
            massthis = new byte[16];
            masspast = new byte[16];
        ...
        System.arraycopy (arr, 0, mass, 0, BLOCK_SIZE);
        mass = GOST_Kuz_Decript(mass);
        mass = mod2(mass, iv);
        System.arraycopy (mass, 0, mas, 0, BLOCK_SIZE);
        return mas;
    }

    public byte[] doFinalenc(byte[] arr)
    {
        int n = arr.length;
        int m = n % BLOCK_SIZE;
        ...
        if (m == 0)
        {
            ...
            byte[] mass = new byte[n + BLOCK_SIZE - m];
            System.arraycopy (arr, 0, mass, 0, n);
            for (int i = n; i < mass.length; i++)
            {
                mass[i] = 0;
            }
            mas = updateenc(mass);
    ...
    public byte[] doFinaldec(byte[] arr)
    {
        ...
        byte[] mas = new byte[i + 1];
        System.arraycopy (mass, 0, mas, 0, mas.length);
        return mas;
    }
